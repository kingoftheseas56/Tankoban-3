...
:: builds Release, then windeployqt so out\Tankoban3.exe launches standalone.
...
cmake --build "%BUILD_DIR%" --config Release --target Tankoban3 > "%LOG%" 2>&1
...
:: Deploy Qt runtime DLLs next to the exe so it launches standalone.
